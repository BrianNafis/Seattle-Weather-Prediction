...
# Prediction button
predict_button = st.sidebar.button("Predict Weather", type="primary", use_container_width=True)
...
with col1:
    ...
    # Create a nice dataframe for input summary
    input_summary = pd.DataFrame({
        "Parameter": ["Precipitation", "Max Temperature", "Min Temperature", "Wind Speed", 
            ...
            if prediction == 1:
                st.markdown("""
                    <div style='background-color: #fff3cd; padding: 20px; border-radius: 10px; text-align: center; border: 2px solid #ffc107'>
                        <h1 style='color: #856404; margin: 0'>SUNNY</h1>
                        <p style='color: #856404; margin: 0'>The day is predicted to be sunny</p>
                    </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown("""
                    <div style='background-color: #f8d7da; padding: 20px; border-radius: 10px; text-align: center; border: 2px solid #dc3545'>
                        <h1 style='color: #721c24; margin: 0'>NOT SUNNY</h1>
                        <p style='color: #721c24; margin: 0'>The day is predicted to be not sunny</p>
                    </div>
                """, unsafe_allow_html=True)
            
            if hasattr(model, 'predict_proba'):
                prob = model.predict_proba(input_df)[0][1]
                st.markdown(f"**Sunny Probability:** {prob*100:.1f}%")
                st.progress(float(prob))
        else:
            st.info("Click the 'Predict Weather' button on the sidebar to get prediction.")
